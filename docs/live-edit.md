...
builder. When it is switched on, every block that opts in shows a small gear at its top-start
corner. Clicking it opens a bottom sheet with the block's live-editable properties, rendered with
the same widgets the builder's property panel uses. Saving writes into `builder_pages.components`
