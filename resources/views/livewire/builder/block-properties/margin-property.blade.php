<div class="form-control w-full">
    <label class="label">
        ...
        <button wire:click="testMethod" class="btn btn-xs btn-primary">Test</button>
    </label>
                ...
                <div class="grid grid-cols-3 gap-3 max-w-80 mx-auto mb-4">
                    ...
                    <div class="text-center">
                        <select wire:model.live="unifiedClassValues.top"
                            ...
                            <option value="">-</option>
                            @foreach ($property['marginClasses'] as $class => $label)
                                @if ($class !== '')
                    ...
                    <div class="bg-secondary/10 border-2 border-dashed border-secondary/30 rounded flex items-center justify-center text-xs text-secondary font-medium">
                        Element
                    </div>
    ...
    @else
        ...
        <div class="space-y-3">
            ...
            <div class="bg-base-200 rounded-lg p-4">
                <div class="text-xs font-medium text-base-content/60 mb-3 uppercase tracking-wider">
                    {{ ucfirst($activeDevice) }} Margin
                </div>

                @php
                    $currentValues = [];
                    switch($activeDevice) {
                        case 'mobile': $currentValues = $mobileClassValues; break;
                        case 'tablet': $currentValues = $tabletClassValues; break;
                        case 'desktop': $currentValues = $desktopClassValues; break;
                        default: $currentValues = ['top' => '', 'right' => '', 'bottom' => '', 'left' => ''];
                    }
                    $currentCustomValues = [];
                    switch($activeDevice) {
                        case 'mobile': $currentCustomValues = $mobileCustomValues; break;
                        case 'tablet': $currentCustomValues = $tabletCustomValues; break;
                        case 'desktop': $currentCustomValues = $desktopCustomValues; break;
                        default: $currentCustomValues = ['top' => '', 'right' => '', 'bottom' => '', 'left' => ''];
                    }
                @endphp
                ...
                <div class="grid grid-cols-3 gap-3 max-w-80 mx-auto mb-4">
                    ...
                    <div class="text-center">
                        <select wire:change="updateIndividualValue('{{ $activeDevice }}', 'top', $event.target.value)"
                                class="select select-bordered w-full text-sm">
                            <option value="" {{ ($currentValues['top'] ?? '') === '' ? 'selected' : '' }}>-</option>
                            @foreach ($property['marginClasses'] as $class => $label)
                                @if ($class !== '')
                                    <option value="{{ $class }}" {{ ($currentValues['top'] ?? '') === $class ? 'selected' : '' }}>{{ $label }}</option>
                                @endif
                            @endforeach
                        </select>
                        @if (($currentValues['top'] ?? '') === 'custom')
                            <input type="number"
                                   wire:model.live="{{ $activeDevice }}CustomValues.top"
                                   class="input input-bordered w-full text-center mt-1 text-sm"
                                   placeholder="0"
                                   min="0">
                        @endif
                    ...
                    <div class="text-center">
                        <select wire:change="updateIndividualValue('{{ $activeDevice }}', 'left', $event.target.value)"
                                class="select select-bordered w-full text-sm">
                            <option value="" {{ ($currentValues['left'] ?? '') === '' ? 'selected' : '' }}>-</option>
                            @foreach ($property['marginClasses'] as $class => $label)
                                @if ($class !== '')
                                    <option value="{{ $class }}" {{ ($currentValues['left'] ?? '') === $class ? 'selected' : '' }}>{{ $label }}</option>
                                @endif
                            @endforeach
                        </select>
                        @if (($currentValues['left'] ?? '') === 'custom')
                            <input type="number"
                                   wire:model.live="{{ $activeDevice }}CustomValues.left"
                                   class="input input-bordered w-full text-center mt-1 text-sm"
                                   placeholder="0"
                                   min="0">
                        @endif
                    ...
                    <div class="text-center">
                        <select wire:change="updateIndividualValue('{{ $activeDevice }}', 'right', $event.target.value)"
                                class="select select-bordered w-full text-sm">
                            <option value="" {{ ($currentValues['right'] ?? '') === '' ? 'selected' : '' }}>-</option>
                            @foreach ($property['marginClasses'] as $class => $label)
                                @if ($class !== '')
                                    <option value="{{ $class }}" {{ ($currentValues['right'] ?? '') === $class ? 'selected' : '' }}>{{ $label }}</option>
                                @endif
                            @endforeach
                        </select>
                        @if (($currentValues['right'] ?? '') === 'custom')
                            <input type="number"
                                   wire:model.live="{{ $activeDevice }}CustomValues.right"
                                   class="input input-bordered w-full text-center mt-1 text-sm"
                                   placeholder="0"
                                   min="0">
                        @endif
                    ...
                    <div class="text-center">
                        <select wire:change="updateIndividualValue('{{ $activeDevice }}', 'bottom', $event.target.value)"
                                class="select select-bordered w-full text-sm">
                            <option value="" {{ ($currentValues['bottom'] ?? '') === '' ? 'selected' : '' }}>-</option>
                            @foreach ($property['marginClasses'] as $class => $label)
                                @if ($class !== '')
                                    <option value="{{ $class }}" {{ ($currentValues['bottom'] ?? '') === $class ? 'selected' : '' }}>{{ $label }}</option>
                                @endif
                            @endforeach
                        </select>
                        @if (($currentValues['bottom'] ?? '') === 'custom')
                            <input type="number"
                                   wire:model.live="{{ $activeDevice }}CustomValues.bottom"
                                   class="input input-bordered w-full text-center mt-1 text-sm"
                                   placeholder="0"
                                   min="0">
                        @endif
                ...
                @if (in_array('custom', [$currentValues['top'] ?? '', $currentValues['right'] ?? '', $currentValues['bottom'] ?? '', $currentValues['left'] ?? '']))
                    <div class="text-center text-xs text-base-content/50">
                        Values in {{ $property['unit'] }}
                    </div>
                @endif
