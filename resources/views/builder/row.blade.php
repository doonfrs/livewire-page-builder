<div class="grid grid-cols-12 mb-2 ">
    <div x-data="{ selected: false }" class="{{ $cssClasses }} group">
        <div
            ...
            <div class="absolute top-[-35px] left-1/2 -translate-x-1/2 bg-pink-500 shadow-lg px-1 py-1 rounded-lg flex items-center space-x-1 z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none group-hover:pointer-events-auto">
                <!-- Delete Button -->
                <button 
                    wire:click="$dispatch('deleteRow', {rowId: '{{$rowId}}'})" 
                ...
                <button 
                    @click="$dispatch('toggle-row-options', {rowId: '{{$rowId}}'})"
                    class="w-7 h-7 flex items-center justify-center text-white hover:bg-pink-600 rounded transition-colors duration-150"
                    title="More Options">
            ...
            <div 
                ...
                <div 
                    ...
                    <div class="px-3 py-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">Row Actions</div>
